...
- `src/main.py` — точка входа
  ...
  - Инициализация логирования согласно политике из @vision.md

- `src/bot/handlers.py` — обработка сообщений
  - Обработчики команд: `/start`, `/help`
  - Обработчик текстовых сообщений
  - Управление краткой историей диалога в памяти процесса (dict/list)
  - Логирование входящих сообщений, ответов и ошибок

- `src/llm/client.py` — работа с LLM (OpenRouter)
  - Формирование и отправка запросов к OpenRouter через OpenAI-совместимый клиент
  - Обработка ответов (контент, токены, метаданные при необходимости)
  - Простая обработка ошибок (сетевые, rate limits, недоступность сервиса) с логированием
  - Интерфейс, удобный для использования из хэндлеров

- `src/config.py` — конфигурация
  - Загрузка настроек из переменных окружения `.env` (см. раздел Конфигурация)
  - Валидация обязательных параметров, значения по умолчанию для необязательных
  - Предоставление типизированного объекта/функций доступа к конфигурации

- `src/bot/__init__.py` и `src/llm/__init__.py`
  - Маркировка пакетов, возможная экспозиция основных интерфейсов модулей
